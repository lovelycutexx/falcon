`timescale 1ns/1ps
// Minimal test: depth=1. Root SPLIT → leaf SAMPLE → ADJUST at root → MERGE at root.
// Same constant signal: t0=DC=2, t1=DC=4 (FFT of [1,1],[2,2])
// C golden: z0≈2.83, z1≈4.0, norm≈20

module tb_ffsampling_d1;
    reg clk, rst_n;
    reg [255:0] mem [0:255];
    ...
    initial begin
        ...
        tw_rom_re[255]=$realtobits(1.0); tw_rom_im[255]=$realtobits(0.0);
    end
    ...
    always @(posedge clk) begin
        if (fe_wr_en) begin
            ...
            $display("[%0t] WR[%0d]=re=%016x im=%016x", $time, fe_wr_addr, fe_wr_data[63:0], fe_wr_data[127:64]);
        end
        if (fe_sz_cmd_valid && fe_sz_cmd_ready)
            $display("[%0t] SZ mu=%016x", $time, sz_cmd_mu);
    end
    ...
    initial begin
        ...
        ts_cfg_depth=1; ts_cfg_dynamic=0;
        ts_t_base=0; ts_tree_base=16; ts_z_base=0; ts_tmp_base=32;
        fpu_val_q=0;

        for (i=0;i<256;i=i+1) mem[i]=256'd0;

        // t0: FFT([1,1]) → DC=2, size n=2
        mem[0]={128'd0,$realtobits(0.0),$realtobits(2.0)};
        // t1: FFT([2,2]) → DC=4
        mem[2]={128'd0,$realtobits(0.0),$realtobits(4.0)};  // component_words=2

        // Tree: L10 at node, all fpr_of(1.0)
        for (i=0;i<3;i=i+1) mem[16+i]={128'd0,$realtobits(0.0),$realtobits(1.0)};

        #20 rst_n=1; #10;
        @(posedge clk);
        $display("=== ffSampling depth=1, constant signal ===");
        ts_start=1; @(posedge clk); ts_start=0;

        wait(ts_done); @(posedge clk);
        $display("=== Done ===");
        $display("z0: re=%016x im=%016x", mem[0][63:0], mem[0][127:64]);
        $display("z1: re=%016x im=%016x", mem[2][63:0], mem[2][127:64]);
        $finish;
    end
endmodule
